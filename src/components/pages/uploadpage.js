    ...
    this.state.web3.eth.getAccounts((error, accounts) => {
      this.state.audioStorage
        ...
        .then(result => {
          ...
          console.log(
            "TX Receipt for createAudioEntry on UploadPage.js: ",
            result
          );
        });
    });
        ...
        <main className="container">
          <div className="pure-g">
            ...
            <div className="pure-u-1-2">
              <UploadForm onAdd={uploadObject => this.onAdd(uploadObject)} />
            </div>

            <div className="pure-u-1-2">
              {this.state.uploadObject.name &&
              this.state.uploadObject.artist ? (
                <p>
                  {this.state.uploadObject.artist} -{" "}
                  {this.state.uploadObject.name}
                </p>
              ...
              {this.state.uploadObject.audioHash ? (
                <p>Audio Hash: {this.state.uploadObject.audioHash}</p>
              ) : (
                <p />
              )}
              {this.state.uploadObject.imageHash ? (
                <p>Image Hash: {this.state.uploadObject.imageHash}</p>
              ) : (
                <p />
              )}
              {this.state.txReceipt ? (
                <p>Tx Receipt: {this.state.txReceipt}</p>
              ) : (
                <p />
              )}
            </div>
